...
$hotelReceipts = [];
$hotelStmt = mysqli_stmt_init($conn);
if (mysqli_stmt_prepare($hotelStmt, "SELECT booking_id, name, email, hotel_name, city_name, tourists, number_of_rooms, room_type, total_amount, booking_date, check_in_date, check_out_date FROM hotel_bookings WHERE userid = ?")) {
    mysqli_stmt_bind_param($hotelStmt, "i", $userid);
    mysqli_stmt_execute($hotelStmt);
    $hotelResult = mysqli_stmt_get_result($hotelStmt);
    while ($row = mysqli_fetch_assoc($hotelResult)) {
...
$tourBookings = [];
$tourStmt = mysqli_stmt_init($conn);
if (mysqli_stmt_prepare($tourStmt, "SELECT booking_id, name, email, city_name, tour_name, tourists, tour_date, contact, price_per_person, total_amount, booking_date FROM tour_bookings WHERE userid = ?")) {
    mysqli_stmt_bind_param($tourStmt, "i", $userid);
    mysqli_stmt_execute($tourStmt);
    $tourResult = mysqli_stmt_get_result($tourStmt);
    while ($row = mysqli_fetch_assoc($tourResult)) {
